...
**Branch:** `agent/clean-room-evaluation-corpus`, based on `master` at
`2bdcea6` (merged QA PR #24)
**Working slice:** PR D, a licensed clean-room evaluation corpus. No tracing
algorithm, configuration default, threshold, Cargo dependency, or strict
synthetic fixture changed.

## PR D — clean-room evaluation corpus

The earlier attempt built the slice in a temporary checkout, then stalled while
transferring blobs through GitHub. The checkout disappeared on the context
reset before any branch, commit, or PR reached GitHub. Five generated source
images survived; the manifest, tooling, and analytic assets did not. This slice
restored from `master`, retained those five rather than regenerating them, and
rebuilt only the deterministic half.

The corpus contains 18 fixed inputs under `engine/fixtures/`: 13 first-party
analytic/adversarial PNGs generated by
`tools/make_evaluation_corpus.py`, and five first-party generated naturalistic
inputs (one actual JPEG container and four PNGs). The manifest records a fixed
6/6/6 train/development/holdout split, SHA-256, provenance, intended profiles,
known features, protected topology, exclusions, gates, renderer/container
details, and source-vector availability. Generated inputs are blind evaluation
material, never mathematical truth and never threshold calibration evidence.

The supplied presentation sheets remain outside the repository. The retained
assets contain no third-party logo, known character, text, or identifiable
landmark. Because the original image-generation call records were lost with
the temporary session, `fixtures/prompts/README.md` labels the prompt records
as faithful reconstructions rather than exact reproduction recipes.

The validator checks both classes without a new codec dependency: PNG decoding
reuses the engine's first-party standard-library decoder, JPEG structure and
dimensions are parsed directly, all digests are fixed, analytic regeneration
must be byte-identical, and declared alpha/palette/rarity/grid properties are
verified. Four unit tests include negative controls for a missing required
colour, a one-byte mutation, and hidden RGB lost under alpha zero.

One visual-QA command accidentally overwrote the last analytic PNG by making it
an ImageMagick output. The next validation caught the digest mismatch; running
the committed generator restored the exact original digest. Do not reuse that
montage command with a repository input as an ambiguous final argument.

Commands run:

```
python3 engine/tools/validate_evaluation_corpus.py
    18 valid fixtures (6/6/6); 13 regenerate byte-identically; 5 fixed digests
python3 -m unittest engine/tools/test_evaluation_corpus.py -v
    4 passed, including all three negative controls
make engine-evaluation-corpus
    validates all 18 fixtures, then runs the same 4 tests successfully
git diff --check
    clean
```

`cargo`, `rustc`, `cargo-deny`, and `gh` are absent in this environment. Rust
gates were therefore not rerun; current `master`'s PR #24 handoff records 423
tests plus two doctests, clean Clippy/fmt/WASM, and 13-fixture native/WASI
parity. PR D changes no Rust, Cargo metadata, strict fixture, or engine output.

Next: build the blind SVG scorer and differential harness, then run PTE,
VTracer, Potrace, AutoTrace, and ImageTracerJS against this exact corpus without
giving any engine privileged analytic truth. Do not tune thresholds from the
holdout split.

## PR #24 QA retained context

QA of merged PR #23 (§11.7 circle recognition) found seven defects, two
blocking. The Kåsa fit and its refusal gates were sound; what followed them was
not. The earlier slices below remain because their decisions are still context.
...
**The clean-room evaluation corpus now exists.** PR D recreated the supplied
test-card intentions as raw first-party inputs with accurate alpha/container
semantics and explicit manifests. The inspected presentation sheets remain
outside the repository. Five naturalistic inputs are generated rather than
captured photographs; retain that limitation in any benchmark claim.
