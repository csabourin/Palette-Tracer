...
class _ThreadingHTTPServer(socketserver.ThreadingMixIn, HTTPServer):
    ...
    This prevents a slow pipeline run from blocking every other request
    (status polls, asset fetches) and eliminates the 502 timeouts seen when
    the proxy gives up waiting for a single-threaded server.
    """
    daemon_threads = True
...
def launch_palette_trace_app(
    ...
    server = _ThreadingHTTPServer((host, port), PaletteTraceRequestHandler)
