...
| Large uploads are traced at reduced resolution | §9.4.2, §17.4 | `server/uploads.py::MAX_WORKING_PIXELS` downscales uploads above 4 MP because the pipeline runs every mask operation at full source resolution. A 12 MP phone photo is otherwise slow enough to look hung. The interface states the traced dimensions | Implementing §17.4 preview scaling would let the trace run at full resolution while previews stay cheap, and this cap could then be raised or dropped |
